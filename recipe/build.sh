...
# We also hack the FORTRANFLAGS and FFLAGS to remove the arch specification which does
# not seem to work.
if [[ "$CONDA_BUILD_CROSS_COMPILATION" == "1" ]] && [[ `uname -s` == "Darwin" ]]; then
    ...
    tmp=`echo $FFLAGS | sed -e 's/-march[^ ]* //g'`
    export FFLAGS=$tmp
    tmp=`echo $FORTRANFLAGS | sed -e 's/-march[^ ]* //g'`
    export FORTRANFLAGS=$tmp
fi
